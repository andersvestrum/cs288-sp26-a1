...
class PerceptronModel:
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Epoch {epoch+1}/{num_epochs} validation accuracy: {100 * val_acc:.2f}%")
            if val_acc > best_val_acc:
                best_val_acc = val_acc
                best_weights = dict(self.weights)

        if best_weights is not None:
            self.weights = defaultdict(float, best_weights)

        print(f"Training complete. Best validation accuracy: {100 * best_val_acc:.2f}%")
